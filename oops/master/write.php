				...
				<div class="box-body">
					...
					<div class="form-group">
						...
						<div class="input-group">
							...
							<input type="text" class="form-control" name="hp" id="hp" value="<?=$hp?>" placeholder="" data-inputmask='"mask": "999-9999-9999"' data-mask required>
						</div>
					</div>
					<div class="form-group">
					  ...
					  <div class="radio">
						<?if($SUPER_ULEVEL == "1") {?>
						<label>
						<input type="radio" name="grade" id="grade0" class="flat-red" value="1" <?=$grade=="1"?"checked":""?>> 슈퍼 관리자
						</label>
						<?}?>
					...
					<div class="form-group">
					  ...
					  <table width="100%" border="0" cellspacing="0" cellpadding="0">
						...
						while ($menu_arr1 = mysql_fetch_array($menu_rs1)) {
							...
							$sub_q1 = "SELECT count(userid) as bcnt FROM ".$initial."_menu_grade WHERE userid='$userid'";
							$sub_q1 .= " AND bid='$bid'";
						...
						?>
						<tr>
							<td><input type="checkbox" name="bmenu[]" id="bmenu_<?=$bid?>" value="<?=$bid?>" style="border:0px;" <?=$bcnt == 0?"":"checked"?>  onclick="fn_GradeChk('<?=$bid?>')"><b <?=$bcnt == 0?"":"style='color:#FF3300;'"?>> <label for="bmenu_<?=$bid?>"><?=$bmenu_name?></label></b></td>
							...
							while ($menu_arr2 = mysql_fetch_array($menu_rs2)) {
								...
								$sub_q2 = "SELECT count(userid) as bcnt FROM ".$initial."_menu_grade WHERE userid='$userid'";
								$sub_q2 .= " AND mid='$mid'";
								...
								?>
									<td width="160"><input type="checkbox" name="mmenu_<?=$bid?>[]" id="mmenu_<?=$mid?>" value="<?=$mid?>" style="border:0px;" <?=$mcnt == 0?"":"checked"?>> <span  <?=$mcnt == 0?"":"style='color:#FF3300;'"?>><label for="mmenu_<?=$mid?>"><?=$mmenu_name?></label></span>&nbsp;</td>
								<?
									$j++;
								}
								?>
					...
					<div class="form-group">
					  ...
					  <div class="radio">
						<label>
							<input type="radio" name="active" id="active1" class="flat-red" value="1" <?if($active=="1"){?>checked<?}?>> 승인
						</label>
						<label>
							<input type="radio" name="active" id="active2" class="flat-red" value="0" <?if($active=="0" || $active == ""){?>checked<?}?>> 거부
						</label>
					  </div>
					</div>
