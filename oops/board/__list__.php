			  ...
			  <tbody>
				...
				<?}?>
                  <td nowrap class="title">
					...
					<?}?>
					<?=$n_depth_str?> <span><a href="javascript:go_view('<?=$n_idx?>')";><?=$n_title?></a> <?=$n_new_img?></span>
				...
				<?}?>
                  <td nowrap class="title">
					...
					<?}?>
					<?=$depth_str?> <?=$secret_img?><a href="javascript:go_view('<?=$idx?>')";><?=$title?></a>&nbsp;<?=$new_img?> <?if($cnt > 0) {?>[<span style="font-size:0.9em;color:#cc3300;"><?=$cnt?></span>]<?}?>
				  </td>
				<?if(!preg_match("/file/i", $cfg_item_close)) {?>
                  <td nowrap class="download">
				  <?if($file_cnt) {?>
